...
if __name__ == '__main__':
    ...
    parser.add_argument("--lr_list", help='comma delimited list input', type=str, default='0.001')
    ...
    # lr_list = [float(item) for item in args.lr_list.split(',') if item]
    lr_list = list(np.round(np.arange(0.001,0.09,0.001),4))

    for n, lr in enumerate(lr_list,1):

        version = args.version+str(n)
        ...
        for dset_sz in pbar1:
            ...
            for train_idx, val_idx in  pbar2:
                ...
                for n_t, theta in pbar3:
                    ...
                    if out_fold==5 and fold==5 and np.mean(REG_net)>0.735:
                        print("LR FOUND:",lr)
                        print("Exiting")
                        break